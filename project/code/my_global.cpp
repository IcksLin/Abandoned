...
RedRectDetector red_detector(UVC_WIDTH,UVC_HEIGHT,0.58);        // 图像裁剪器
LQ_NCNN ncnn_classifier;                                        // 目标分类组件，龙邱方案

// --- 目标分类投票器静态变量 ---
int frame_count = 0;
std::map<std::string, float> confidence_accumulator; // 用于累加各类的置信度
const int MAX_ACCUMULATE_FRAMES = 15;
const float CONFIDENCE_THRESHOLD = 0.65f;
float limit_left   = 0.15f; // 左边界：距左侧 20% 宽度
float limit_right  = 0.15f; // 右边界：距右侧 20% 宽度 (即 1 - 0.2 = 80%)
float limit_up     = 0.20f; // 上边界：距顶部 20% 高度
float limit_down   = 0.20f; // 下边界：距底部 20% 高度
...
void pid_contol_handle()
{
    ...
    // static float tick_codex = 0;
    // tick_codex ++;
    // temp_speed = 50*sin(tick_codex/100);
    // motor_set_speed(temp_speed,temp_speed);
    // printf("right: %f    ,left: %f   \r",right_speed,left_speed);
