...
MyPID angle_speed_pd;                                           // 三串角速度环，限幅CRUISING_SPEED*0.6,-CRUISING_SPEED*0.6,输入参数onto_control_pd计算结果
MyPID onto_control_pd;                                          // 三串方向环，限幅-700~700，输入参数 onto，范围+-90
...
void hight_frequence_encoder_get_speed_handler(){
    ...
    if(onto_pd_control_enable==1){
        ...
        target_speed_l = target_speed_r = 1;//添加基准速度(调角速度环用的)
...
{   
    ...
    if(onto_pd_control_enable==1){
        ...
        // 角速度环调试
        // onto_control = angle_speed_pd.control(0,cur_angle_speed);
